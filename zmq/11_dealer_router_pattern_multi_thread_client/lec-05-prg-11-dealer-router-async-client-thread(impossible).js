...
const sock = makeASocket("dealer", process.argv[2], frontAddr, "connect");

// 마스터 프로세스의 경우에 아래의 코드를 수행한다.
if (cluster.isMaster) {
  // 새로운 프로세스를 이용하여 하나의 소켓에 대해 sender와 receiver를 생성한다.
  cluster.fork({ TYPE: "sender" });
  cluster.fork({ TYPE: "receiver" });
} else {
  if (process.env.TYPE === "sender") {
    clientTask(sock); // 메시지를 보내는 이벤트가 있다.
  } else if (process.env.TYPE === "receiver") {
    recvHandler(sock); // 메시지를 받는 이벤트 리스너가 있다.
  }
}
